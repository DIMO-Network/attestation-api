// Code generated by protoc-gen-go. DO NOT EDIT.
// versions:
// 	protoc-gen-go v1.32.0
// 	protoc        v4.25.2
// source: pkg/grpc/atttestation-api.proto

package grpc

import (
	protoreflect "google.golang.org/protobuf/reflect/protoreflect"
	protoimpl "google.golang.org/protobuf/runtime/protoimpl"
	reflect "reflect"
	sync "sync"
)

const (
	// Verify that this generated code is sufficiently up-to-date.
	_ = protoimpl.EnforceVersion(20 - protoimpl.MinVersion)
	// Verify that runtime/protoimpl is sufficiently up-to-date.
	_ = protoimpl.EnforceVersion(protoimpl.MaxVersion - 20)
)

type BatchCreateVINVCRequest struct {
	state         protoimpl.MessageState
	sizeCache     protoimpl.SizeCache
	unknownFields protoimpl.UnknownFields

	TokenIds []uint32 `protobuf:"varint,1,rep,packed,name=tokenIds,proto3" json:"tokenIds,omitempty"`
	Force    bool     `protobuf:"varint,2,opt,name=force,proto3" json:"force,omitempty"`
}

func (x *BatchCreateVINVCRequest) Reset() {
	*x = BatchCreateVINVCRequest{}
	if protoimpl.UnsafeEnabled {
		mi := &file_pkg_grpc_atttestation_api_proto_msgTypes[0]
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		ms.StoreMessageInfo(mi)
	}
}

func (x *BatchCreateVINVCRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*BatchCreateVINVCRequest) ProtoMessage() {}

func (x *BatchCreateVINVCRequest) ProtoReflect() protoreflect.Message {
	mi := &file_pkg_grpc_atttestation_api_proto_msgTypes[0]
	if protoimpl.UnsafeEnabled && x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use BatchCreateVINVCRequest.ProtoReflect.Descriptor instead.
func (*BatchCreateVINVCRequest) Descriptor() ([]byte, []int) {
	return file_pkg_grpc_atttestation_api_proto_rawDescGZIP(), []int{0}
}

func (x *BatchCreateVINVCRequest) GetTokenIds() []uint32 {
	if x != nil {
		return x.TokenIds
	}
	return nil
}

func (x *BatchCreateVINVCRequest) GetForce() bool {
	if x != nil {
		return x.Force
	}
	return false
}

type BatchCreateVINVCResponse struct {
	state         protoimpl.MessageState
	sizeCache     protoimpl.SizeCache
	unknownFields protoimpl.UnknownFields

	Results []*VINVCResult `protobuf:"bytes,1,rep,name=results,proto3" json:"results,omitempty"`
}

func (x *BatchCreateVINVCResponse) Reset() {
	*x = BatchCreateVINVCResponse{}
	if protoimpl.UnsafeEnabled {
		mi := &file_pkg_grpc_atttestation_api_proto_msgTypes[1]
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		ms.StoreMessageInfo(mi)
	}
}

func (x *BatchCreateVINVCResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*BatchCreateVINVCResponse) ProtoMessage() {}

func (x *BatchCreateVINVCResponse) ProtoReflect() protoreflect.Message {
	mi := &file_pkg_grpc_atttestation_api_proto_msgTypes[1]
	if protoimpl.UnsafeEnabled && x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use BatchCreateVINVCResponse.ProtoReflect.Descriptor instead.
func (*BatchCreateVINVCResponse) Descriptor() ([]byte, []int) {
	return file_pkg_grpc_atttestation_api_proto_rawDescGZIP(), []int{1}
}

func (x *BatchCreateVINVCResponse) GetResults() []*VINVCResult {
	if x != nil {
		return x.Results
	}
	return nil
}

type VINVCResult struct {
	state         protoimpl.MessageState
	sizeCache     protoimpl.SizeCache
	unknownFields protoimpl.UnknownFields

	TokenId uint32  `protobuf:"varint,1,opt,name=tokenId,proto3" json:"tokenId,omitempty"`
	RawVC   string  `protobuf:"bytes,2,opt,name=rawVC,proto3" json:"rawVC,omitempty"`
	Error   *string `protobuf:"bytes,3,opt,name=error,proto3,oneof" json:"error,omitempty"`
}

func (x *VINVCResult) Reset() {
	*x = VINVCResult{}
	if protoimpl.UnsafeEnabled {
		mi := &file_pkg_grpc_atttestation_api_proto_msgTypes[2]
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		ms.StoreMessageInfo(mi)
	}
}

func (x *VINVCResult) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*VINVCResult) ProtoMessage() {}

func (x *VINVCResult) ProtoReflect() protoreflect.Message {
	mi := &file_pkg_grpc_atttestation_api_proto_msgTypes[2]
	if protoimpl.UnsafeEnabled && x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use VINVCResult.ProtoReflect.Descriptor instead.
func (*VINVCResult) Descriptor() ([]byte, []int) {
	return file_pkg_grpc_atttestation_api_proto_rawDescGZIP(), []int{2}
}

func (x *VINVCResult) GetTokenId() uint32 {
	if x != nil {
		return x.TokenId
	}
	return 0
}

func (x *VINVCResult) GetRawVC() string {
	if x != nil {
		return x.RawVC
	}
	return ""
}

func (x *VINVCResult) GetError() string {
	if x != nil && x.Error != nil {
		return *x.Error
	}
	return ""
}

var File_pkg_grpc_atttestation_api_proto protoreflect.FileDescriptor

var file_pkg_grpc_atttestation_api_proto_rawDesc = []byte{
	,
}

var (
	file_pkg_grpc_atttestation_api_proto_rawDescOnce sync.Once
	file_pkg_grpc_atttestation_api_proto_rawDescData = file_pkg_grpc_atttestation_api_proto_rawDesc
)

func file_pkg_grpc_atttestation_api_proto_rawDescGZIP() []byte {
	file_pkg_grpc_atttestation_api_proto_rawDescOnce.Do(func() {
		file_pkg_grpc_atttestation_api_proto_rawDescData = protoimpl.X.CompressGZIP(file_pkg_grpc_atttestation_api_proto_rawDescData)
	})
	return file_pkg_grpc_atttestation_api_proto_rawDescData
}

var file_pkg_grpc_atttestation_api_proto_msgTypes = make([]protoimpl.MessageInfo, 3)
var file_pkg_grpc_atttestation_api_proto_goTypes = []interface{}{
	(*BatchCreateVINVCRequest)(nil),  // 0: grpc.BatchCreateVINVCRequest
	(*BatchCreateVINVCResponse)(nil), // 1: grpc.BatchCreateVINVCResponse
	(*VINVCResult)(nil),              // 2: grpc.VINVCResult
}
var file_pkg_grpc_atttestation_api_proto_depIdxs = []int32{
	2, // 0: grpc.BatchCreateVINVCResponse.results:type_name -> grpc.VINVCResult
	0, // 1: grpc.AttestationService.BatchCreateVINVC:input_type -> grpc.BatchCreateVINVCRequest
	1, // 2: grpc.AttestationService.BatchCreateVINVC:output_type -> grpc.BatchCreateVINVCResponse
	2, // [2:3] is the sub-list for method output_type
	1, // [1:2] is the sub-list for method input_type
	1, // [1:1] is the sub-list for extension type_name
	1, // [1:1] is the sub-list for extension extendee
	0, // [0:1] is the sub-list for field type_name
}

func init() { file_pkg_grpc_atttestation_api_proto_init() }
func file_pkg_grpc_atttestation_api_proto_init() {
	if File_pkg_grpc_atttestation_api_proto != nil {
		return
	}
	if !protoimpl.UnsafeEnabled {
		file_pkg_grpc_atttestation_api_proto_msgTypes[0].Exporter = func(v interface{}, i int) interface{} {
			switch v := v.(*BatchCreateVINVCRequest); i {
			case 0:
				return &v.state
			case 1:
				return &v.sizeCache
			case 2:
				return &v.unknownFields
			default:
				return nil
			}
		}
		file_pkg_grpc_atttestation_api_proto_msgTypes[1].Exporter = func(v interface{}, i int) interface{} {
			switch v := v.(*BatchCreateVINVCResponse); i {
			case 0:
				return &v.state
			case 1:
				return &v.sizeCache
			case 2:
				return &v.unknownFields
			default:
				return nil
			}
		}
		file_pkg_grpc_atttestation_api_proto_msgTypes[2].Exporter = func(v interface{}, i int) interface{} {
			switch v := v.(*VINVCResult); i {
			case 0:
				return &v.state
			case 1:
				return &v.sizeCache
			case 2:
				return &v.unknownFields
			default:
				return nil
			}
		}
	}
	file_pkg_grpc_atttestation_api_proto_msgTypes[2].OneofWrappers = []interface{}{}
	type x struct{}
	out := protoimpl.TypeBuilder{
		File: protoimpl.DescBuilder{
			GoPackagePath: reflect.TypeOf(x{}).PkgPath(),
			RawDescriptor: file_pkg_grpc_atttestation_api_proto_rawDesc,
			NumEnums:      0,
			NumMessages:   3,
			NumExtensions: 0,
			NumServices:   1,
		},
		GoTypes:           file_pkg_grpc_atttestation_api_proto_goTypes,
		DependencyIndexes: file_pkg_grpc_atttestation_api_proto_depIdxs,
		MessageInfos:      file_pkg_grpc_atttestation_api_proto_msgTypes,
	}.Build()
	File_pkg_grpc_atttestation_api_proto = out.File
	file_pkg_grpc_atttestation_api_proto_rawDesc = nil
	file_pkg_grpc_atttestation_api_proto_goTypes = nil
	file_pkg_grpc_atttestation_api_proto_depIdxs = nil
}
